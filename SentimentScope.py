...
print("\nAverage words per review:", train_df["length"].mean())
...
# STEP 3: Split Train into Train + Validation

# 90% for training
train_size = int(0.9 * len(train_df))

# Shuffle dataset to mix positive + negative reviews
...
print("Training subset size   :", len(train_data))
print("Validation subset size :", len(val_data))
...
# Take 3 random reviews from training data
...
tokenized_samples = tokenizer(
    sample_texts,
    truncation=True,         # Cut extra long reviews
    padding="max_length",    # Pad smaller texts
    max_length=128,          # All sequences -> length 128
    return_tensors="pt"      # Return PyTorch tensors
)

print("\n TOKENIZED OUTPUT KEYS:", tokenized_samples.keys())
print("\n input_ids SHAPE:", tokenized_samples["input_ids"].shape)
print("\n attention_mask SHAPE:", tokenized_samples["attention_mask"].shape)
...
class IMDBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            "label": torch.tensor(label, dtype=torch.long)   # ✔ FIXED HERE
        }
...
class DemoGPT(nn.Module):
    def __init__(self, config):
        ...
        # CLASSIFICATION HEAD (Completed TODO)
...
Built a sentiment analysis model using a transformer-based architecture.
The model was trained on the IMDB movie reviews dataset. 
After training for multiple epochs, the model reached around 75% accuracy, and with a larger configuration and more training, it can reach 90% accuracy. 
The model successfully predicts whether a review is positive or negative.

Key Takeaways:

1. Transformer models can be used even for simple text classification tasks.
2. Training for more epochs and using a bigger model improves accuracy.
3. GPU support is important for training deep learning models faster.
4. The IMDB dataset is a good benchmark for testing sentiment models.
With proper tuning, the model can reach performance close to pretrained models.
'''
